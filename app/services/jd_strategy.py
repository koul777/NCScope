...
def build_strategy_with_openai(
    ...
    api_key = str(api_key_override or "").strip() or settings.openai_key()
    default_target = int(os.getenv("INTERVIEW_TARGET_COUNT", "10") or "10")
    ...
    retry_target_count = max(1, min(10, target_count))
    ...
    if isinstance(question_plan, dict):
        plan_items = [
            ...
            if isinstance(x, dict) and str(x.get("detail", "")).strip()
        ]
    ...
    prompt = (
        ...
        "\n"
        "[꼬리질문 작성 기준]\n"
        "- 꼬리물기 구조: 주질문 이후 각 꼬리질문이 앞 답변을 전제로 더 깊이 파고드는 질문\n"
        "- 꼬리질문은 각각 evaluation_points의 서로 다른 항목을 검증\n"
