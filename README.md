...
### v1.4.4 · 2026-08-16 · 반복형 STAR 문구 완화 및 질문 변형 강화
- 질문 생성 프롬프트에서 고정 STAR 단서문 반복 의존도를 낮추고, 같은 KSA라도 사건 맥락·제약·산출물 포인트를 매 요청 다르게 구성하도록 지시를 추가했습니다.
- 공식 근거는 KSA/evidence 기준으로 유지하되, 입력 문구를 그대로 복붙한 질문 패턴이 생성되지 않도록 다양성 규칙을 강화했습니다.
- 면접기법은 KSA와 함께 입력받아, STAR를 정답 형식이 아닌 답변 프레임으로만 해석해 질문을 생성합니다.
...
- 면접기법과 KSA를 기준으로 생성되며, STAR는 질문 본문 고정 문구가 아닌 답변 구도 가이드로만 적용됩니다. 따라서 같은 KSA를 다시 생성해도 사건 배경·위험 요소·우선순위 조건을 바꿔서 다른 문항으로 출력하려고 시도합니다.
...
## run_question_quality_loop (질문 리스트 즉시 확인)
```powershell
