...
- OpenRouter는 프롬프트·응답을 기본적으로 저장하지 않는다고 안내하지만, 요청 메타데이터는 처리·보관될 수 있습니다. [OpenRouter Data Collection](https://openrouter.ai/docs/guides/privacy/data-collection)
- OpenRouter는 요청을 모델 제공업체로 전달하며, 제공업체별 보관·학습 정책이 다를 수 있습니다. [Provider Logging](https://openrouter.ai/docs/guides/privacy/provider-logging/)
- 보존을 더 엄격히 제한해야 하는 기관은 OpenRouter 계정·요청에서 Zero Data Retention(ZDR) 제공업체만 허용하는 정책을 확인하십시오. [Zero Data Retention](https://openrouter.ai/docs/guides/features/zdr)
- 외부 AI 전송이 기관 정책상 금지되거나 자료에 개인정보·민감정보·비공개 보안정보가 포함된 경우 OpenRouter 생성 기능을 사용하지 마십시오.

OpenRouter API 키는 브라우저 저장소가 아니라 Vercel Production 서버 환경변수에서 관리합니다. 이 키 보호와 업로드 문서 경계는 [`SECURITY.md`](SECURITY.md)도 함께 확인하십시오.
...
공식 서비스와의 관계, 데이터 사용, 면접 운영 책임에 대한 고지는 [`NOTICE.md`](NOTICE.md)를 참고하세요. 서버·요청 단위 API 키 처리와 업로드 문서 기준은 [`SECURITY.md`](SECURITY.md)에 정리했습니다. Kordoc 등 외부 구성요소 고지는 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)에 분리했습니다.

화면은 네 단계로 구성됩니다.

1. 서버 OpenRouter 연결 상태 확인
2. 직무기술서 세분류 검토
3. 공고문 핵심텍스트 보완
...
- 일반 문항은 서버가 고정한 OpenRouter `stealth/ox-alpha`를 medium 추론으로 최대 8초 시도하고, 발표·토론·인바스켓·창의적 문제해결력 또는 복합 질문 계획은 high 추론으로 최대 15초 시도합니다. 시간 초과 또는 사용할 수 없는 JSON은 서버가 지정한 `OPENROUTER_RECOVERY_MODEL`로 복구합니다.
- 정상 생성 경로에서는 선택한 세분류의 공식 NCS KSA, 면접 형태 1개, 공고·직무기술서 맥락을 모델에 전달합니다. 모델이 주질문·꼬리질문·평가포인트를 직접 작성하며, 서버가 STAR 문구나 공통 질문 골격을 덧붙이거나 모델 문장을 결정적 템플릿으로 교체하지 않습니다.
- 공식 NCS KSA `evidence_id`는 서버 내부에서 질문 슬롯과 정확히 결합합니다. 지원자용 질문에 KSA 라벨이나 원문 단어가 그대로 나타날 필요는 없으며, 품질 검사는 해당 KSA를 의미상 관찰할 수 있는지, 선택한 면접 형태에 맞는지, 블라인드 채용·근거·중복 기준을 지키는지를 확인합니다.
- 두 외부 모델이 모두 실패한 비상 경로에서만 조회·검증된 공식 KSA에 잠긴 서버 폴백을 `human_review_required`로 반환할 수 있습니다. 이 결과는 모델 생성 완료 문항으로 표시하지 않으며 공급자 예외나 키 원문을 포함하지 않습니다.
- 공개 운영 프로필은 한 요청에 주질문을 최대 5개까지 허용합니다. 6개 이상 계획은 Kordoc·NCS·모델 호출 전에 HTTP 422 `question_plan_capacity_exceeded`로 거부하고, 추가 문항은 결과 화면의 다른 질문 생성 기능으로 이어서 생성합니다.
...
### v1.4.4 · 2026-08-23 · ALIO 문서 복구와 KSA 기반 AI 자유 질문 생성

- Kordoc이 ALIO/HWP-to-PDF 표의 분류 열을 비워 반환하는 경우, PDF 구조 기반 공식 NCS 후보 추출을 보조 경로로 실행해 세분류 검토 드롭다운을 복구합니다. Kordoc 후보가 있으면 기존 결과를 우선 보존합니다.
- `03. NCS 기반 채용 직무 설명자료 (16).pdf` 실문서를 운영 엔드포인트로 재현해 HTTP 200과 세분류 후보 8개를 확인했습니다.
- ALIO 최신 공고 3건을 실제로 내려받아 파싱한 결과 3건 모두 문서 파싱에 성공했고, 1건은 세분류 2개를 추출했습니다. 나머지 2건은 원문에 명시적 NCS 세분류가 없어 자동 후보를 만들지 않고 사람 검토 대상으로 남깁니다. 상세 결과는 `reports/alio_jd_benchmark_20260823_145324_777162-bc023292.md`에 기록했습니다.
- ALIO 최신 공고 8건을 운영 NCS MCP와 함께 재검증했습니다. 8/8 문서 파싱 성공, 명시적 세분류 문서 4건, 정확 매칭 1건·부분 매칭 1건·미매칭 3건으로 집계됐으며, 세분류가 없는 4건은 자동 질문 생성을 차단하고 검토 사유를 남겼습니다. 상세 결과는 `reports/alio_jd_benchmark_20260823_150605_185317-798a8a73.md`입니다.
- 같은 8건에 대한 템플릿 품질 루프도 실행했습니다. 평가 진입은 1/8건이었고, 결정적 서버 fallback 문항은 `human-review-only`로 유지되어 준비 완료로 오판하지 않았습니다. 상세 결과는 `reports/alio_question_quality_20260823_150443_753842-bbcb3ba7.md`입니다.
- 경험·상황·발표·토론·인바스켓·직무지식·창의적 문제해결력면접 7개 경로가 동일한 런타임 품질 오케스트레이터, 중복 차단, 원문 누출 검사, KSA 재검사를 통과하도록 회귀 계약을 추가했습니다.
- 선택한 공식 KSA 원문·과업 의미·관찰 행동과 공고·직무기술서 맥락을 모델 입력으로 제공하고, 모델이 면접 형태에 맞춰 주질문·꼬리질문·평가포인트를 자유롭게 작성하도록 변경했습니다. STAR·`문서·수치·기록·피드백`·`확인·판단 기준` 같은 공통 문장 골격은 질문에 강제하지 않습니다.
- OpenRouter가 작성한 문항은 KSA 라벨 단어가 보이지 않는다는 이유로 서버 템플릿으로 교체하지 않습니다. 서버가 검증한 정확한 `evidence_id`를 평가 근거로 유지하면서 의미상 측정성·면접기법·안전·중복을 검사하고, 실패 슬롯은 같은 KSA로 모델에 다시 작성시킵니다.
- 검증 결과: PDF/NCS 파서·오케스트레이션 대상 `118 passed`, 전 면접형태 오케스트레이션 계약 `182 passed`.

### v1.4.3 · 2026-08-15 · 대량 질문 생성 복구와 질문 품질 개선
...
- 경험면접 주질문과 답변 연동형 꼬리질문이 선택한 KSA의 행동 증거를 빠짐없이 확인하도록 보강하고, 평가포인트도 질문에서 실제로 확인할 수 있는 관찰 증거에 맞췄습니다.
...
- 요청 단위 생성 API 키는 현재 탭 메모리에만 유지하고 생성 요청마다 전달합니다. UI에 키 지우기와 HTTPS 전송 경고를 추가하고, query string·로그·DB·응답에 키가 남지 않도록 경계를 강화했습니다.
...
- 당시 공개 UI와 API의 생성 경로를 `openai_api` 요청 단위 키로 고정했습니다. 현재 운영 배포는 아래의 서버 관리 OpenRouter 키 정책을 따릅니다.
...
- 당시에는 브라우저 키 입력을 제거했고 이후 요청 단위 BYOK를 거쳤습니다. 현재 운영 계약은 서버 관리 OpenRouter 키와 선택적 요청 키이며, 최신 지침은 아래와 [`SECURITY.md`](SECURITY.md)를 따릅니다.
...
1. `.env`가 있으면 설정을 불러옵니다. `OPENROUTER_ALLOW_SERVER_KEY=true`와 유효한 `OPENROUTER_API_KEY`가 함께 있으면 로컬에서도 서버 키를 사용합니다. `OPENAI_API_KEY`는 서버 fallback으로 읽지 않습니다.
...
`provider_fallback_used=true`, `question_release_status=human_review_required`로 표시됩니다.

개인 키를 입력하면 해당 생성 요청에서만 서버 키보다 우선합니다. `sk-or-...`는
OpenRouter, 다른 `sk-...`는 OpenAI로 감지하며 JSON body 또는 multipart form의
`generation_api_key`로만 전달합니다. URL query에는 넣을 수 없고 브라우저 저장소,
파일, DB, 로그, 감사로그 또는 응답에 저장하지 않습니다.

`GET /api/generation-provider/status`는 키 값을 받거나 외부 인증을 시도하지 않습니다.
서버 키가 준비되면 `provider=openrouter_api`, `auth_mode=server_env_api_key`,
`status=configured`, `credential_configured=true`, `authenticated=false`를 반환합니다.
마지막 값은 상태 조회가 실제 키를 외부로 보내 검증하지 않는다는 뜻입니다.

공개 운영 프로필은 한 요청에 주질문을 최대 5개까지 허용합니다. 6개 이상은 외부 작업 전에
HTTP 422 `question_plan_capacity_exceeded`로 거부됩니다. 품질 실패 시 한 번의 제한된
재생성을 사용할 수 있지만, 이 호출도 별도 시간을 추가하지 않고 같은 285초 예산을
공유합니다. 실제 사용 횟수와 상한은 응답의 `strategy.model_quality_retry`와 화면 품질
안내에 비밀정보 없이 표시됩니다. 외부 생성이 끝내 실패하면 공식 KSA가 있는 요청은
서버 KSA 폴백으로 전환되며, 모델이 만든 불합격 문장은 사용자에게 노출하지 않습니다.
...
### 6. 공공기관 공고문 업로드와 핵심 텍스트 보완

공개 UI에서는 공공기관 채용공고문과 해당 공고의 NCS 기반 직무기술서를 함께
준비해야 합니다. 직무기술서는 공식 NCS 세분류·KSA 근거를 잡고, 공고문은 실제
담당업무·지원자격·우대사항·면접 평가 맥락을 보완합니다. 두 문서를 모두 올린 뒤
추출 내용을 검토·적용해야 질문 생성 버튼이 활성화됩니다.
...
NCScope는 모델이 선택한 KSA와 면접기법을 해석해 질문 문구를 자유롭게 작성하게 하고, 생성 결과를 면접기법별 품질 게이트로 다시 점검합니다.
...
과거 검증 리포트는 deterministic template fallback 품질과 model-origin 질문 품질을 분리해 집계했습니다. 현재 공개 생성 경로는 요청 키로 선택된 OpenRouter Ox Alpha 또는 OpenAI 모델 결과만 허용하고 provider 또는 최종 품질 게이트 실패를 템플릿으로 대체하지 않습니다. `model_quality_passed`는 본질문과 후속질문이 모두 모델 산출물로 보존된 질문만 통과로 셉니다.
...
NCScope UI
  ↓ 요청마다 generation_api_key를 body/form으로 전달
FastAPI(키를 파일·DB·로그에 저장하지 않음)
  ├─ Kordoc 문서 파싱
  ├─ Human review gate
  ├─ NCS_MCP_URL → 로컬 NCS DB 검색 서버에서 공식 NCS/KSA 조회
  └─ sk-or- → 고정 OpenRouter Ox Alpha / sk- → 승인된 OpenAI gateway
```
...
FastAPI 앱 import 시점에는 `.env`를 자동으로 읽지 않습니다. 로컬 실행은 `.\run_local.ps1`을 권장합니다. 이 스크립트는 `.env`를 현재 프로세스에만 읽어 들이며, OpenRouter 서버 키는 `OPENROUTER_ALLOW_SERVER_KEY=true`일 때만 활성화됩니다. `.env`는 Git에 커밋하지 마세요.
...
실제 키는 문서·명령행·셸 기록에 넣지 말고 로컬 `.env` 또는 배포 플랫폼의 비밀변수로 관리하세요. 개인 키 입력 기능을 사용하는 외부 배포는 HTTPS가 먼저 구성되어 있어야 합니다.
...
| `INTERVIEW_GENERATION_PROVIDER` | 선택 | `openrouter_api` | 기본 생성 provider. 요청 키가 있으면 접두사와 provider를 다시 검증 |
| `OPENROUTER_ALLOW_SERVER_KEY` | 선택 | `false` | `true`일 때만 서버 관리 OpenRouter 키 사용 |
| `OPENROUTER_API_KEY` | 운영 시 조건부 | 없음 | OpenRouter 서버 키. Vercel Production secret 또는 커밋되지 않는 로컬 `.env`에 저장 |
| `OPENAI_API_KEY` | 사용 안 함 | 없음 | 공개 생성의 서버 fallback으로 사용하지 않음. OpenAI는 요청 단위 키만 허용 |
...
| `GENERATION_MAX_MAIN_QUESTIONS` | 선택 | `5` | 요청당 주질문 상한. Vercel Production은 `5` |
...
현재 Vercel 운영 프로필의 비밀이 아닌 핵심 값은 다음과 같습니다.
...
응답에는 markdown·검토 필드와 함께 파일명·파일 해시에 결합된 서명 검토 세션이
포함됩니다. 공고문은 `POST /api/notice/parse-review`에서 같은 방식으로 검토합니다.
...
- `jd_file`: 원본 직무기술서
- `notice_file`: 공공기관 채용공고문. 공개 UI에서는 필수
- `jd_review_json`: 사람이 확정한 직무기술서 필드와 서명 검토 세션
- `notice_review_json`: 사람이 확정한 공고문 필드와 서명 검토 세션
- `question_plan_json`: 공개 운영에서는 주질문 합계가 1 이하여야 하는 질문 계획
- `generation_provider`: 보통 생략. 지정 시 `openrouter_api` 또는 `openai_api`이며 키 접두사와 일치해야 함
...
`review_confirmed`가 정확히 `true`가 아니거나 세분류가 비어 있으면 로컬 NCS DB 조회를
진행하지 않습니다. 공개 브라우저 흐름은 두 원본 파일과 서명 세션의 파일명·해시를
검증하고, 일치할 때 파싱된 markdown을 다시 추출하지 않고 재사용합니다.
...
```json
{
  "generation_api_key": "<OpenRouter sk-or-... 또는 OpenAI sk-...>",
  "notice_text": "담당업무 ...",
...
모든 생성 endpoint는 `generation_api_key`를 JSON body 또는 multipart form으로 받아
해당 요청에만 사용합니다. 위 값은 형식 표시용 placeholder이며 실제 키를 문서,
테스트, 명령행 또는 로그에 복사하지 마세요. 요청 키를 생략하면 활성화된 Vercel
Production 서버 키를 사용하고, 서버 키도 없을 때만 키 필요 오류를 반환합니다.
OpenRouter/OpenAI 호출 또는 근거·안전 필수 품질 게이트가 실패하면 비밀값과 upstream
원문을 제거하고 공식 KSA 기반 서버 폴백을 시도합니다. 정확한 계획 개수와 공식
`evidence_id`를 모두 충족한 경우에는 검토 필요 문항으로 반환하며, 공식 근거 자체가
없는 경우에만 provider별 5xx 오류로 닫힙니다.
...
Docker 이미지에는 앱만 포함합니다. NCS 데이터 조회는 별도 로컬 NCS DB 검색 서버(NCS_MCP)를 통해 수행합니다. 서버 관리 모드는 `OPENROUTER_ALLOW_SERVER_KEY=true`와 `OPENROUTER_API_KEY`를 컨테이너 secret으로 주입해 사용합니다. 키를 이미지나 명령행에 넣지 마세요. 개인 키 입력 기능을 외부에 노출할 때는 TLS를 강제하고 proxy·WAF·APM의 body 기록을 꺼야 합니다.
